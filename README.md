...
## Install
```
git clone https://github.com/social-robotics-lab/robot-controller-client.git
```

## Docker build and run
```
cd robot-controller-client
...
- ServoMapは軸のラベル(LABEL)と角度の指令値(value)のマップです。軸のラベルはCommUとSotaで異なります。詳細は[各軸の情報](#各軸の情報)を見てください。valueの単位は度で、整数値（int）の値をとります。
